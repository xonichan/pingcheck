...
async def main_loop(dashboard: Dashboard, ping_manager: PingManager, 
    ...
    while True:
        try:
            # Обработка ввода (поддержка английской и русской раскладки)
            key = dashboard.stdscr.getch()
            
            if key == -1:
                pass
            elif key in (ord('q'), ord('й'), 3, ord('x'), ord('ф')):  # q, й, Ctrl+C, x, ф
                break
            elif key in (ord('r'), ord('ц'), ord('R'), ord('Ц')):  # r, ц, R, Ц
                # Перечитать файл
